...
from gevent import socket as socket                  # 用这种类型的socket
from gevent.queue import LifoQueue, Empty, Full      # 用这种类型的阻塞队列
...
class Connection(object):
    ...
    def __init__(self, host='localhost', port=6379, db=0, password=None,
        ...
        self.retry_on_timeout = retry_on_timeout                                  # 如果设置了这个标志位，那么将会在重试的时候考虑超时
    ...
    def connect(self):
        ...
        if self._sock:
            return
        try:
            sock = self._connect()                                # 创建socket
        except socket.error:
            e = sys.exc_info()[1]
            raise ConnectionError(self._error_message(e))

        self._sock = sock                                         # 保存tcp连接
        try:
            self.on_connect()                                     # 如果需要验证的话，那么搞一下
        except RedisError:
        ...
        for callback in self._connect_callbacks:                  # 调用一些连接回调
            callback(self)
    ...
    def on_connect(self):
        ...
        self._parser.on_connect(self)                                # 初始化协议解析
    ...
    def disconnect(self):
        """关闭底层的连接"""
        self._parser.on_disconnect()                    # 通知解析器
        ...
        try:
            ...
            pass
        self._sock = None                              # 销毁底层socket的引用

    def send_packed_command(self, command):
        "Send an already packed command to the Redis server"
        if not self._sock:                                                            # 如果当前底层还没有建立连接，那么建立一个
            self.connect()
        try:
...
class ConnectionPool(object):
    ...
    def __init__(self, connection_class=Connection, max_connections=None,
        ...
        connection_class.
        """
        max_connections = max_connections or 2 ** 31                                 # 默认最大连接数为最大，相当于没有最大
        if not isinstance(max_connections, (int, long)) or max_connections < 0:
            raise ValueError('"max_connections" must be a positive integer')

        self.connection_class = connection_class                                     # 默认使用的连接类型就是上面的Connection
        self.connection_kwargs = connection_kwargs                                   # 一些连接参数，例如ip，端口什么的
        self.max_connections = max_connections                                       # 当前连接池的最大连接数目
    ...
    def get_connection(self, command_name, *keys, **options):
        ...
        self._checkpid()
        try:
            connection = self._available_connections.pop()                          # 获取一个连接
        except IndexError:
            connection = self.make_connection()                                     # 如果没有可用的，那么获取一个新的连接
        self._in_use_connections.add(connection)                                    # 将这个连接放到正在使用的集合中
        return connection

    def make_connection(self):
        """创建一个新的连接"""
        if self._created_connections >= self.max_connections:
            raise ConnectionError("Too many connections")                           # 连接的数目超标了，那么抛出异常
        self._created_connections += 1                                              # 创建的连接数目加1
        return self.connection_class(**self.connection_kwargs)                      # 返回连接对象

    def release(self, connection):
        """当连接使用完了将会调用这个方法来返还连接"""
        self._checkpid()
